...
function [Hessian,gradient,ConstraintMatrix_equality,biasVectorConstraint_equality, ...
          ConstraintMatrix_inequality,biasVectorConstraint_inequality] = ...
          balancingControllerWithChangingWeights(feetInContact, M, h, J, impedances, ...
                                                 dampings, s, sDot, s_sDot_sDDot_des, acc_task_star, ...
                                                 JDot_nu, ConstraintsMatrix_feet, biasVectorConstraint_feet, ...
                                                 w_H_l_sole, w_H_r_sole, Weights, Sat, Config, ROBOT_DOF) 
    ...
    if feetInContact(1) > 0.1
        weightLeftFoot  = Weights.weightStanceFoot;
    else
        weightLeftFoot  = Weights.weightSwingFoot;
    end
    if feetInContact(2) > 0.1
        weightRightFoot  = Weights.weightStanceFoot;
    else
        weightRightFoot  = Weights.weightSwingFoot;
    end
    ...
    if Config.QP_USE_STRICT_TASK_PRIORITIES
        ...
        % Taking off the hands from the tasks
        J_strict = J(1:18,:);
        JDot_nu_strict = JDot_nu(1:18);
        acc_task_star_strict = acc_task_star(1:18);
        
        Hessian  = transpose(St_invM_B)*St_invM_B + Weights.weight_tau*transpose(S_tau)*S_tau;
        gradient =-transpose(St_invM_B)*(St_invM_h + sDDot_star);
        
        % Multiplier of u in the equality constraint equation and bias terms
        J_invM_B  = J_strict*invM*B;
        J_invM_h  = J_strict*invM*h;
        
        % Equality constraints
        ConstraintMatrix_equality     = J_invM_B;
        biasVectorConstraint_equality = acc_task_star_strict - JDot_nu_strict + J_invM_h;
        ...
        Hessian  =   weightLeftFoot          * transpose(J_invM_B_lfoot)  * J_invM_B_lfoot ...
                   ...
                   + Weights.weightCoM       * transpose(J_invM_B_CoM)    * J_invM_B_CoM ...
                   + Weights.weightRotTask   * transpose(J_invM_B_RotTask)* J_invM_B_RotTask ...
                   + Weights.weightLeftHand  * transpose(J_invM_B_lhand)  * J_invM_B_lhand ...
                   + Weights.weightRightHand * transpose(J_invM_B_rhand)  * J_invM_B_rhand ...
                   + Weights.weightPostural  * transpose(St_invM_B)       * St_invM_B ...
                   + Weights.weight_tau      * transpose(S_tau)           * S_tau;
        ...
        gradient =   weightLeftFoot          * transpose(J_invM_B_lfoot)   * (JDot_nu_lfoot - J_invM_h_lfoot - acc_task_star_lfoot) ...
                   ...
                   + Weights.weightCoM       * transpose(J_invM_B_CoM)     * (JDot_nu_CoM - J_invM_h_CoM - acc_task_star_CoM) ...
                   + Weights.weightRotTask   * transpose(J_invM_B_RotTask) * (JDot_nu_RotTask - J_invM_h_RotTask - acc_task_star_RotTask) ...
                   + Weights.weightLeftHand  * transpose(J_invM_B_lhand)   * (JDot_nu_lhand - J_invM_h_lhand - acc_task_star_lhand) ...
                   + Weights.weightRightHand * transpose(J_invM_B_rhand)   * (JDot_nu_rhand - J_invM_h_rhand - acc_task_star_rhand) ...
                   + Weights.weightPostural  * transpose(St_invM_B)        * (- St_invM_h - sDDot_star);        
