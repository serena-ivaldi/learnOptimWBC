...
%% --- Initialization ---
function acc_task_star = setTasksAccelerations(pos_vel_acc_CoM_des, pose_vel_acc_LFoot_des, pose_vel_acc_RFoot_des, orient_vel_acc_rot_task_des, pose_vel_acc_LHand_des,pose_vel_acc_RHand_des, ...
                                               pos_vel_CoM, pose_vel_LFoot, pose_vel_RFoot, orient_vel_rot_task, pose_vel_LHand, pose_vel_RHand, ...
                                               Kp_Kd_CoM, Kp_Kd_LFoot, Kp_Kd_RFoot, Kp_Kd_rot_task, Kp_Kd_LHand, Kp_Kd_RHand,...
                                               feetInContact, Sat) 
    ...
    % LHand position, and linear velocity
    pos_LHand    = pose_vel_LHand(1:3,1);
    vel_LHand    = pose_vel_LHand(1:3,2);
    
    % LHand orientation, and angular velocity
    w_R_LHand    = pose_vel_LHand(4:6,1:3);
    omega_LHand  = pose_vel_LHand(4:6,4);
    
    % RHand position, and linear velocity
    pos_RHand    = pose_vel_RHand(1:3,1);
    vel_RHand    = pose_vel_RHand(1:3,2);
    
    % RHand orientation, and angular velocity
    w_R_RHand    = pose_vel_RHand(4:6,1:3);
    omega_RHand  = pose_vel_RHand(4:6,4);
    ...
    % LHand desired position, linear velocity and acceleration
    pos_LHand_des      = pose_vel_acc_LHand_des(1:3,1);
    vel_LHand_des      = pose_vel_acc_LHand_des(1:3,2);
    acc_LHand_des      = pose_vel_acc_LHand_des(1:3,3);
    
    % LFoot desired orientation, angular velocity and acceleration
    w_R_LHand_des      = pose_vel_acc_LHand_des(4:6,1:3);
    omega_LHand_des    = pose_vel_acc_LHand_des(4:6,4);
    omegaDot_LHand_des = pose_vel_acc_LHand_des(4:6,5);
    
    % RHand desired position, linear velocity and acceleration
    pos_RHand_des      = pose_vel_acc_RHand_des(1:3,1);
    vel_RHand_des      = pose_vel_acc_RHand_des(1:3,2);
    acc_RHand_des      = pose_vel_acc_RHand_des(1:3,3);
    
    % RHand desired orientation, angular velocity and acceleration
    w_R_RHand_des      = pose_vel_acc_RHand_des(4:6,1:3);
    omega_RHand_des    = pose_vel_acc_RHand_des(4:6,4);
    omegaDot_RHand_des = pose_vel_acc_RHand_des(4:6,5);
    ...
    % LHand position and linear velocity gains
    Kp_LHand       = diag(Kp_Kd_LHand(1:3,1));
    Kd_LHand       = diag(Kp_Kd_LHand(1:3,2));
    
    % LFoot orientation and angular velocity gains
    Kp_rot_LHand   = diag(Kp_Kd_LHand(4:6,1));
    Kd_omega_LHand = diag(Kp_Kd_LHand(4:6,2));
    
    % RHand position and linear velocity gains
    Kp_RHand       = diag(Kp_Kd_RHand(1:3,1));
    Kd_RHand       = diag(Kp_Kd_RHand(1:3,2));
    
    % RHand orientation and angular velocity gains
    Kp_rot_RHand   = diag(Kp_Kd_RHand(4:6,1));
    Kd_omega_RHand = diag(Kp_Kd_RHand(4:6,2));
    ...
    % desired left hand acceleration
    acc_LHand_star = linearPID(pos_LHand,vel_LHand,pos_LHand_des,vel_LHand_des,acc_LHand_des,Kp_LHand,Kd_LHand);

    % desired right hand acceleration
    acc_RHand_star = linearPID(pos_RHand,vel_RHand,pos_RHand_des,vel_RHand_des,acc_RHand_des,Kp_RHand,Kd_RHand);
    ...
    if feetInContact(1) > (1-Sat.toll_feetInContact) && Sat.zeroAccWhenFeetInContact
        ...
        omegaDot_LFoot_star = rotationalPID(w_R_LFoot, omega_LFoot, w_R_LFoot_des, omega_LFoot_des, omegaDot_LFoot_des, Kp_rot_LFoot, Kd_omega_LFoot);
    end
    ...
    if feetInContact(2) > (1-Sat.toll_feetInContact) && Sat.zeroAccWhenFeetInContact
        ...
        omegaDot_RFoot_star = rotationalPID(w_R_RFoot, omega_RFoot, w_R_RFoot_des, omega_RFoot_des, omegaDot_RFoot_des, Kp_rot_RFoot, Kd_omega_RFoot);
    end
    
    % desired left hand angular acceleration
    omegaDot_LHand_star = rotationalPID(w_R_LHand, omega_LHand, w_R_LHand_des, omega_LHand_des, omegaDot_LHand_des, Kp_rot_LHand, Kd_omega_LHand);

    % desired right hand angular acceleration
    omegaDot_RHand_star = rotationalPID(w_R_RHand, omega_RHand, w_R_RHand_des, omega_RHand_des, omegaDot_RHand_des, Kp_rot_RHand, Kd_omega_RHand);
    ...
    acc_task_star = [acc_LFoot_star; omegaDot_LFoot_star; ...
                     ...
                     acc_LHand_star; omegaDot_LHand_star; ...
                     acc_RHand_star; omegaDot_RHand_star];        
