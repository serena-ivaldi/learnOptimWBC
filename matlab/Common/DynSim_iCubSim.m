...
function [t, q, qd,failed_flag] = DynSim_iCubSim(controller,params)
    ...
    % i need to set failed_flag to true to initiate the external while and start the simulation
    failed_flag = true; 
    
    while(failed_flag)
        % i would like to repat this cycle only once (because it means that
        % i did not have any trouble). so i set failed_flag
        failed_flag = false;
        %% ---
        % for spacing strings
        s=' ';
        ...
        while(fid == -1)
            ...
            if (timer >= params.max_timer)
                ...
                if(consecutive_fails_counter >= params.max_consecutive_fails_counter)
                            ...
                            pause(3)
                            system('pkill -f gzclient')
                            pause(1)
                            system('pkill -f gzserver')
                            pause(3)
