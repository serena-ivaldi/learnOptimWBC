%% if i use this function matlab crash for no reason (wondenrful!)
function [gp, varargout] = gp_optim_modified(gp, x, y, varargin)
...
w=gp_pak(gp);
%% i changed this line in order to use fmincon or another kind of optimizer with the exact inference method
%if isequal(lower(loss),'e') || (isequal(lower(loss),'loo')) && (isfield(gp.lik.fh,'trcov') || isequal(gp.latent_method, 'EP'))
if (isequal(lower(loss),'loo')) && (isfield(gp.lik.fh,'trcov') || isequal(gp.latent_method, 'EP'))
  switch nargout
    case 6
  ...
  %% changing this bound change a lot the quality of the reconstruction  
  %lb=repmat(-8,size(w));
  %ub=repmat(10,size(w));
  lb=repmat(-100,size(w));
  ub=repmat(100,size(w));
  switch nargout
    case 6
