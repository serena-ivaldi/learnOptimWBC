...
if(part2)
    ...
    % Set the options for optimizer of the acquisition function
    optimf = @fmincon;
    optdefault=struct('GradObj','off','LargeScale','off','Algorithm','trust-region-reflective','TolFun',1e-9,'TolX',1e-6);
    opt=optimset(optdefault);
    ...
    %while i1 < maxiter
        ...
        if i1>1
            gp = gp_optim(gp,x,y);
            % i do not undestand what is that
...
if(part3)
    ...
    Z = reshape(fx(xl),100,100);
    Zc1 = fxc(xl); 
    Zc1(Zc1<const(1,1) | Zc1>const(1,2)) = nan;
...
%     lik = lik_gaussian('sigma2', 0.001, 'sigma2_prior', prior_fixed);
    ...
    gp1 = gp_set('cf', {cfse}, 'lik', lik);
    % GP models for constraint functions
    gpc1 = {gp_set('cf', {cfse}, 'lik', lik, 'jitterSigma2', 1e-6),...
        gp_set('cf', {cfse}, 'lik', lik, 'jitterSigma2', 1e-6)};

    % Set the options for optimizer of the acquisition function
    optimf = @fmincon;
    optdefault=struct('GradObj','off','LargeScale','off','Algorithm','interior-point','TolFun',1e-9,'TolX',1e-6);
    opt=optimset(optdefault);
