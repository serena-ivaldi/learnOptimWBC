...
repetition_of_the_experiment = 100;
 ...
 all_best = zeros(repetition_of_the_experiment,search_space_dimension);
 all_last_best = zeros(repetition_of_the_experiment,search_space_dimension);
 ...
 for jj=1:length(method_to_use)
     for kk = 1:repetition_of_the_experiment
         ...
         exec_time = toc
         all_perfomance(kk,:) = -costs;
         all_best(kk,1) = bestAction.performance;
         [tmp , id]=min(costs(end-last_generation:end));
         ...
         for ii=1:n_constraints
             ...
             all_last_best_violations(kk,ii) = feval(constraints_functions{ii},policies(id,:));
         end
     end
     
     % compute the mean and variance of overall perfomance
     prf.average  = mean(all_perfomance,2); 
     prf.variance = var(all_perfomance,2);
     
     
     
     all_prf{jj} = prf;
     % mean and variance of the best perfomance over all the experiments
     G_best(jj,1) = mean(all_best,2);
     G_best(jj,2) = var(all_best,2);
     % mean and variance of the best perfomance of the last k generations
     % over all the experiments
     G_last_best(jj,1) = mean(all_last_best,2);
     G_last_best(jj,2) = var(all_last_best,2);
     
      for ii=1:n_constraints
         G_best_violations.mean(1,ii) = mean(all_best_violations(:,ii),2); 
         G_best_violations.variance(1,ii) = mean(all_best_violations(:,ii),2); 
         G_last_best_violations.mean(1,ii) = mean(all_last_best_violations(:,ii),2); 
         G_last_best_violations.variance(1,ii) = mean(all_last_best_violations(:,ii),2); 
      end
