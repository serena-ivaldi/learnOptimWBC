...
function CMAESParallelBenchmark
    ...
    for jj=1:number_of_function_2_test
        ...
        parfor kk = 1:repetition_of_the_experiment
            ...
            disp(strcat('********* current experiment = ',num2str(kk), '****** '))
            ...
            if(~robotics_experiment(1,jj))
                % distance from benchmark actions
                m1(kk,1) = norm(bestAction.parameters - benchmark_x);
            end
            % best perfomance
            ...
            m2(kk) = sum(abs((c > 0).*c)) + sum(abs((ceq ~= 0).*ceq));
        end
        close all 
        if(robotics_experiment(jj))
            robotic_flag = true;
        ...
        name_to_save = strcat(local_path,'/',function_2_test{jj},'.mat');
        save(name_to_save,'m1','m2','m3','m4','m5','m6','m7','robotic_flag');
