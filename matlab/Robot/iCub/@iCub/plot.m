...
for i = 1:length(scheme.robot.link)
    if(~strcmp(scheme.robot.link{i}.Attributes.name,'base_link'))
        L{iii} = scheme.robot.link{i}.Attributes.name;  
        iii = iii + 1;
    end
end

% list of joints ( i need this info to build the graph that describe the robot)
for i = 1:length(scheme.robot.joint)
    G{i} = scheme.robot.joint{i};  
end
...
for i = 1:n_plot
    search_string = L{i};
    for j = 1:length(G)
        if(strcmp(G{j}.child.Attributes.link,search_string))
            connection_string = G{j}.parent.Attributes.link;
            type_of_connection = G{j}.Attributes.type;
            index = find(SubStrFind(connection_string,L),1);  
            if(strcmp(type_of_connection,'revolute'))
                A(i,index) = 1;
            elseif(strcmp(type_of_connection,'revolute'))
                A(i,index) = 2;
...
mult_patch(:,1) = mult_patch(:,1)*0.07;
mult_patch(:,2) = mult_patch(:,2)*0.07;
