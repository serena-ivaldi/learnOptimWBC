%CODEGENERATOR.GENMEXJDOT Generate C-MEX-function for Jdot
%
% CGEN.GENMEXJDOT() generates a robot-specific MEX-function to compute
% forward kinematics.
%
% Notes::
...
function [] = genmexJdot(CGen)

%% Forward kinematics up to tool center point
CGen.logmsg([datestr(now),'\tGenerating Jdot MEX-function: ']);
symname = 'jacob_dot';
...
hStruct = createHeaderStructJdot(CGen.rob,symname); 

% Generate and compile MEX function 
CGen.mexfunction(tmpStruct.(symname), 'funfilename',funfilename,'funname',[CGen.getrobfname,'_',symname],'vars',{Q,QD},'output','jacob_dot','header',hStruct)
