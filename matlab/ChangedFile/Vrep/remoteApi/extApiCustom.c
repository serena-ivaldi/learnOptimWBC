...
EXTAPI_DLLEXPORT simxInt simxCustomGetTime(simxInt clientID,simxFloat* time,simxInt operationMode)
{
	...
	/* Now "execute" the command (i.e. place the command (with its associated command data) into the outbox, and retrieve a command reply from the inbox) */
    dataPointer=_exec_null(clientID,simx_customcmd_get_time,operationMode,0,&returnValue); /* _int indicates that the command is identified by the command ID AND an integer value*/
...
EXTAPI_DLLEXPORT simxInt simxCustomGetDelta(simxInt clientID,simxFloat* delta,simxInt operationMode)
{
	...
	/* Now "execute" the command (i.e. place the command (with its associated command data) into the outbox, and retrieve a command reply from the inbox) */
    dataPointer=_exec_null(clientID,simx_customcmd_get_delta,operationMode,0,&returnValue); /* _int indicates that the command is identified by the command ID AND an integer value*/
