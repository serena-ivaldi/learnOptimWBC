...
function [p,pd,pdd,time] = Circular(s,time_struct,geom_parameters,type)
   ...
   rad_val      = geom_parameters(1); %  length of radius
   phi_val      = geom_parameters(2); %  pan
   theta_val    = geom_parameters(3); %  tilt
   wzero_val    = geom_parameters(4); % starting point on the circle
   x_centre_val = geom_parameters(5); % x coordinate of the centre
   y_centre_val = geom_parameters(6); % y coordinate of the centre
   z_centre_val = geom_parameters(7); % z coordinate of the centre
