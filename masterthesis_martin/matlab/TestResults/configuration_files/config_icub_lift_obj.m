% Lift-object experiment configuration script:
%
...
%
% Note: This configuration file works only if the WBM-Toolbox is installed
%       on the system.
...
% Selected controller type ('GHC' or 'UF'):
CONTROLLERTYPE = 'UF';
...
% Backup file for the current parameters:
...
% add it to robot list of the learning-framework:
...
%
% GRAB_OBJECT_AT_POS:
trg_pts{1,1}  = [0.20   0.06  0.56]; % move r_hand to targ. pos. A
trg_pts{1,2}  = [0.20  -0.06  0.56]; % move l_hand to targ. pos. B
% SET_JOINT_POSITIONS:
...
trg_pts{1,4}  = [0.20   0.06  0.76]; % move r_hand to interm. pos. C
trg_pts{1,5}  = [0.20  -0.06  0.76]; % move l_hand to interm. pos. D
% MOVE_OBJECT_TO_POS (final target points):
trg_pts{1,6}  = [0.35   0.06  0.76]; % move r_hand to targ. pos. E
trg_pts{1,7}  = [0.35  -0.06  0.76]; % move l_hand to targ. pos. F
% RELEASE_OBJECT:
% MOVE_HANDS_TO_POS (release obj.):
trg_pts{1,8}  = [0.10   0.11  0.76]; % move r_hand to pos. G
trg_pts{1,9}  = [0.10  -0.11  0.76]; % move l_hand to pos. H
...
switch CONTROLLERTYPE
    case 'UF'
        ...
        rep_subchain         = 7;
        rep_target_link{1,1} = rep_subchain;
...
% iCub simulation - initial configuration:
...
% Note: This chain list is not defined in joint configurations of
%       the Yarp-WholeBodyInterface [1], but its components are
...
switch CONTROLLERTYPE
    case 'UF'
        ...
        % Note: The reference parameter works only, if at least one of the
        %       specified trajectories has some runtime parameters given.
        numeric_reference_parameter{1,1} = [0.047180 0.359539 1.045565 0.374223 -0.069047 0.013630 -0.495463 ...
        ...
        %       index order to the linked repeller objects must also changed to
        %       the same order.
        ...
        % Note: If the secondary trajectories are not needed, then the
        %       secondary task gains will also not used.
        kp = [];
        kd = [];
        ...
        %
        % Note: The regulation terms transform the UF into a regularized UF by
        %       activating a damped least square method (regularization) inside
        %       of the controller, if the values are different from zero.
        %
        %       The regularizer term is an array of vectors with regulation
