function sim_config = initSimConfigICub_liftObj(urdf_file_name, vb_objects, scn_mode, show_light)
    if isempty(vb_objects)
        error('initSimConfigICub_liftObj: %s', WBM.wbmErrorMsg.EMPTY_DATA_TYPE);
    end
    dark_scn = false;
    ...
    sim_body.draw_prop.joints.marker_sz = 16;
    sim_body.draw_prop.joints.color     = WBM.wbmColor.orange;

    env_settings = WBM.wbmSimEnvironment;
    env_settings.grnd_shape   = [-0.5  -0.5  0.5   0.5;  % x
                                 -0.5   0.5  0.5  -0.5;  % y
                                    0     0    0     0]; % z
    env_settings.orig_pt_size = 4.5;

    % Draw the given geometric volume bodies in the
    % environment for the scenario to be simulated:
    ...
    nax = 1;
    sim_config = WBM.genericSimConfig('iCub-Simulator:', sim_body, nax, env_settings);
    % set new window and axes settings ...
    sim_config.wnd_pos     = [200 200]; % [x (left), y (bottom)]
    sim_config.wnd_size    = [600 800]; % [width, height]
    sim_config.axes_pos    = [0.02  0.1  0.96  0.84]; % [x (left), y (bottom), wid (width), hgt (height)]
